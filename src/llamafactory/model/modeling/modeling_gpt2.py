...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.c_proj._is_attention_output = True  # 标记为注意力输出层
...
class GPT2MLP(nn.Module):
    def __init__(self, intermediate_size, config):
        ...
        self.c_proj._is_mlp_output = True  # 标记为MLP输出层
...
class GPT2LMHeadModel(GPT2PreTrainedModel, GenerationMixin):
    ...
    # --- Helper function to prepare inputs for GPT-2 ---
    def prepare_gpt_inputs_for_stages(
        ...
        scale_emb_val: Optional[torch.Tensor], # scale_emb_val is not used in GPT-2's version but kept for signature compatibility
        config_obj # config_obj is not used in GPT-2's version but kept for signature compatibility
    ):
        ...
        return loop_input_embeds_final, loop_pos_ids, loop_attn_mask

    # --- Helper function to extract hidden states for interpolation/refinement ---
    def extract_hidden_for_computation(self, gpt_outputs, selector_slice, config_obj): # config_obj not used but kept for signature
        last_h_state = gpt_outputs.last_hidden_state # For GPT-2, gpt_outputs is from self.transformer
    ...
    def compute_interpolated_embeds(self, weight: torch.Tensor, hidden: Union[torch.Tensor, Tuple[torch.Tensor, ...]], use_topk: bool = True) -> torch.Tensor:
        # This function is adapted from GPTNeoXForCausalLM
        # It uses self.lm_head for logits instead of self.embed_out
        softmax_temp = getattr(self.config, 'softmax_temperature', 1.0)
        hidden_layer_step = getattr(self.config, 'hidden_layer_num', 1)
        if hidden_layer_step <= 0: hidden_layer_step = 1

        if use_topk:
            top_k_val = getattr(self.config, 'interpolation_topk', 100)
            if isinstance(hidden, torch.Tensor):
                logits = self.lm_head(hidden) # Changed from self.embed_out
                ...
                for layer_output in selected_layer_outputs:
                    logits = self.lm_head(layer_output) # Changed from self.embed_out
                    logits_scaled = logits / softmax_temp
                    actual_top_k = min(top_k_val, logits_scaled.size(-1))
            ...
            if isinstance(hidden, torch.Tensor):
                logits = self.lm_head(hidden) / softmax_temp # Changed from self.embed_out
                ...
                sum_probs = None
                for layer_output in selected_layer_outputs:
                    current_logits = self.lm_head(layer_output) / softmax_temp # Changed from self.embed_out
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        global_step: Optional[int] = None, # Added global_step
    ) -> Union[Tuple, CausalLMOutputWithCrossAttentions]:
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        use_cache = use_cache if use_cache is not None else self.config.use_cache # User's cache preference for final pass
        # User's preference for attentions/hidden_states for final pass
        user_requested_output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        ...
        if getattr(self.config, "recurrent_model", False):
            # ---- Recurrent model logic inspired by GPTNeoXForCausalLM ----

            if inputs_embeds is None:
                if input_ids is None: raise ValueError("input_ids must be provided if inputs_embeds is not.")
                initial_embeds_raw = self.transformer.wte(input_ids) # GPT-2's word token embeddings
            else:
                initial_embeds_raw = inputs_embeds

            B, L_orig, H = initial_embeds_raw.shape # H is config.n_embd for GPT-2
            ...
            if attention_mask is None:
                ...
                if attention_mask.ndim == 4: 
                     original_attention_mask_2d = attention_mask[:, 0, 0, :L_orig] 
                elif attention_mask.ndim == 3:
                     original_attention_mask_2d = attention_mask[:, 0, :L_orig] 
            ...
            weight_for_interpolation = self.transformer.wte.weight # GPT-2's wte weight
            embed_scale = None # embed_scale is specific to NeoX config, not directly used here unless scale_embeds is true
            
            if hasattr(self.config, 'scale_embeds') and self.config.scale_embeds:
                # Using H (n_embd) for scaling if config.scale_embeds is True
                if self.config.interpolation == True:
                    embed_scale_factor = torch.sqrt(torch.tensor(H, dtype=initial_embeds_raw.dtype, device=device))
                else:
                    embed_scale_factor = torch.sqrt(2.5*torch.tensor(H, dtype=initial_embeds_raw.dtype, device=device))
                inputs_embeds0 = initial_embeds_raw * embed_scale_factor
                weight_for_interpolation = self.transformer.wte.weight * embed_scale_factor
            ...
            for iter_k_idx in range(num_initial_interpolation_stages): 
                ...
                iter_input_embeds, iter_pos_ids, iter_attn_mask = self.prepare_gpt_inputs_for_stages(
                    current_stages_for_iter_pass, B, L_orig, H, device,
                    original_repeated_position_ids, original_attention_mask_2d,
                    None, self.config # embed_scale and config_obj not used by GPT-2 prepare_gpt_inputs
                )
                
                iter_gpt_outputs = self.transformer(
                    inputs_embeds=iter_input_embeds,
                    attention_mask=iter_attn_mask, 
                    # position_ids=iter_pos_ids, 
                    token_type_ids=None, # Not used in this recurrent flow
                    head_mask=head_mask, past_key_values=None, use_cache=False, # No cache for intermediate
                    output_attentions=False, # No attentions for intermediate
                ...
                for stage_slice_idx in range(num_stages_in_current_iter_input):
                    ...
                    # Pass iter_gpt_outputs (transformer output) to extract_hidden_for_computation
                    hidden_input_for_this_slice = self.extract_hidden_for_computation(
                        iter_gpt_outputs, current_slice_selector, self.config
                    )
                    if self.config.interpolation == True:
                ...
                if iter_k_idx == 0:
                    all_embeddings_stages.append(computed_embeddings_from_hs_slices[0])
                else:
                    for j in range(iter_k_idx): 
            ...
            if self.training:
                num_refinement_steps = getattr(self.config, 'training_refinement_steps', 10)
                if getattr(self.config, "vary_refine_steps", False):
                    # num_refinement_steps = torch.randint(2, 10, (1,)).item()            
                    num_refinement_steps = torch.randint(2, 4, (1,)).item()            

            else: # Evaluation
                num_refinement_steps = getattr(self.config, 'eval_refinement_steps', 0) # Default to 0
            ...
            if num_initial_interpolation_stages > 0 and num_refinement_steps > 0:
                for ref_idx in range(num_refinement_steps):
                    ...
                    ref_input_embeds, ref_pos_ids, ref_attn_mask = self.prepare_gpt_inputs_for_stages(
                        current_stages_for_refinement_pass, B, L_orig, H, device,
                        original_repeated_position_ids, original_attention_mask_2d,
                        None, self.config # embed_scale and config_obj not used by GPT-2 prepare_gpt_inputs
                    )

                    ref_gpt_outputs = self.transformer(
                    ...
                    for i in range(num_initial_interpolation_stages): 
                        ...
                        hidden_input_for_refine_fn = self.extract_hidden_for_computation(
                            ref_gpt_outputs, refine_source_selector, self.config
                        )
                        if self.config.interpolation == True:
            ...
            final_interleaved_input_embeds, final_interleaved_pos_ids, final_interleaved_attn_mask = \
                self.prepare_gpt_inputs_for_stages(
                    final_stages_for_pass, B, L_orig, H, device,
                    original_repeated_position_ids, original_attention_mask_2d,
                    None, self.config) # embed_scale and config_obj not used by GPT-2 prepare_gpt_inputs
            
            final_pass_gpt_outputs = self.transformer(
                ...
                token_type_ids=token_type_ids, # Pass original token_type_ids for final pass
                head_mask=head_mask, 
                past_key_values=past_key_values, # User's cache for final pass
                use_cache=use_cache, # User's cache preference
                output_attentions=user_requested_output_attentions, 
                output_hidden_states=user_requested_output_hidden_states,
                return_dict=True,
            )
            ...
            all_hidden_states_from_final_pass = final_pass_gpt_outputs.last_hidden_state

            logits_selector_start_index = num_initial_interpolation_stages 
            if num_initial_interpolation_stages == 0 and num_total_stages_final == 1: 
                logits_selector_start_index = 0

            if logits_selector_start_index < num_total_stages_final :
                hidden_states_for_target_stage = all_hidden_states_from_final_pass[:, logits_selector_start_index::num_total_stages_final, :]
                final_output_logits = self.lm_head(hidden_states_for_target_stage)
            ...
            # ---- Zero-Overhead Consistency Loss Calculation ----
            if self.training and getattr(self.config, 'consistency_weight', 0) > 0 and num_initial_interpolation_stages > 0:
                lambda_cons = self.config.consistency_weight
                
                # 以最后一个插值阶段为例计算一致性损失
                # 这是在进入 final pass 前计算的 embedding
                target_embedding_for_consistency = all_embeddings_stages[-1]

                # 它的源隐状态来自于倒数第二个阶段
                source_hs_slice_idx = num_initial_interpolation_stages - 1
                
                consistency_source_selector = slice(source_hs_slice_idx, None, num_total_stages_final)
                
                hidden_for_consistency_check = self.extract_hidden_for_computation(
                    final_pass_gpt_outputs, consistency_source_selector, self.config
                )

                # 使用 final pass 的隐状态重新计算 embedding
                interp_use_topk = getattr(self.config, 'interpolation_use_topk', False)
                if self.config.interpolation == True:
                    recomputed_embedding = self.compute_interpolated_embeds(
                        weight_for_interpolation, hidden_for_consistency_check, use_topk=interp_use_topk
                    )
                else:
                    recomputed_embedding = hidden_for_consistency_check

                # --- Relative Consistency Loss Implementations ---
                # You can choose one of the following loss functions.
                
                # Option 1: Normalized L2 Loss
                # This loss is relative to the magnitude of the embeddings. It penalizes the squared difference
                # normalized by the sum of squared magnitudes, preventing the model from simply shrinking embeddings to reduce loss.
                # eps = 1e-6
                # diff_sq = (target_embedding_for_consistency - recomputed_embedding).pow(2).sum(dim=-1)
                # denom = target_embedding_for_consistency.pow(2).sum(dim=-1) + eps
                # consistency_loss = (diff_sq / denom).mean()
                with torch.cuda.amp.autocast(enabled=False):
                    target_fp32 = target_embedding_for_consistency.float()
                    recomputed_fp32 = recomputed_embedding.float()

                    # 1. Directional Loss (Cosine Similarity)
                    # This term is insensitive to vector norm and focuses purely on alignment.
                    consistency_loss = 1 - torch.nn.functional.cosine_similarity(target_fp32, recomputed_fp32, dim=-1).mean()
                #cosine loss
                # consistency_loss = 1 - torch.nn.functional.cosine_similarity(target_embedding_for_consistency, recomputed_embedding, dim=-1).mean()
                
                # 保存consistency loss用于wandb logging
                self.last_consistency_loss = (lambda_cons * consistency_loss).detach().cpu().item()

                if loss is not None:
                    loss += lambda_cons * consistency_loss
                else:
                    loss = lambda_cons * consistency_loss
            else:
                # 如果没有计算consistency loss，设为None
                self.last_consistency_loss = None
            
            final_hidden_states_to_return = final_pass_gpt_outputs.hidden_states if user_requested_output_hidden_states else None
            final_attentions_to_return = final_pass_gpt_outputs.attentions if user_requested_output_attentions else None
            final_past_key_values = final_pass_gpt_outputs.past_key_values if use_cache else None
            
            if not return_dict:
                output_items = [final_output_logits]
                if final_past_key_values is not None: output_items.append(final_past_key_values)
                if final_hidden_states_to_return is not None: output_items.append(final_hidden_states_to_return)
                if final_attentions_to_return is not None: output_items.append(final_attentions_to_return)
                # Let's build the tuple carefully:
                returned_tuple = (final_output_logits,)
                if final_past_key_values is not None:
                    returned_tuple += (final_past_key_values,)
                else: # Need to add placeholder if not returned by model but expected by tuple structure
                     if use_cache : returned_tuple += (None,)


                if final_hidden_states_to_return is not None:
                    returned_tuple += (final_hidden_states_to_return,)
                else: # Placeholder if not returned but expected
                    if user_requested_output_hidden_states : returned_tuple += (None,)

                if final_attentions_to_return is not None:
                    returned_tuple += (final_attentions_to_return,)
                else: # Placeholder if not returned but expected
                    if user_requested_output_attentions : returned_tuple += (None,)
                
                # Cross attentions would be next if it was supported, add None for it if expected by original tuple structure
                # However, the CausalLMOutputWithCrossAttentions definition is what dictates fields.
                # Let's just use the items we have.

                return ((loss,) + returned_tuple) if loss is not None else returned_tuple
            ...
            return CausalLMOutputWithCrossAttentions(
                ...
                cross_attentions=None # Not computed by this recurrent logic
            )
    @staticmethod
    def _reorder_cache(
