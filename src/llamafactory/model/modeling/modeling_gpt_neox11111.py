...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.ponder_temp_init = float(getattr(self.config, "ponder_temp_init", 1.0))
        self.ponder_temp_final = float(getattr(self.config, "ponder_temp_final", 0.01))
        self.ponder_temp_warmup_ratio = float(getattr(self.config, "ponder_temp_warmup_ratio", 0.1))  # 前一段保持 init
        self.ponder_temp_schedule = getattr(self.config, "ponder_temp_schedule", "cosine")  # 'cosine' | 'linear' | 'constant'
        self.ponder_temp = getattr(self.config, "ponder_temperature", 1)

        self.ponder_ent_lambda_start = getattr(self.config, "ponder_ent_lambda_start", 0.0)
        self.ponder_ent_lambda_max   = getattr(self.config, "ponder_ent_lambda_max",   0.0)
        self.ponder_ent_warmup_steps = getattr(self.config, "ponder_ent_warmup_steps", 1000)
        self.ponder_ent_peak_steps   = getattr(self.config, "ponder_ent_peak_steps",   4000)
        # self.lambda_entropy = getattr(self.config, "lambda_entropy", 0.01)
        self.lambda_ponder_cost = getattr(self.config, "lambda_ponder_cost", 0.05)
        self.lambda_ent_w = getattr(self.config, "lambda_ent_w", 0.01)
        self.post_init()        
    ...
    def prepare_gpt_inputs_for_stages(
        ...
        if (w_var is not None) and (num_stages_to_interleave > 1):
            for i in range(1, num_stages_to_interleave):
                wi = w_var[..., i-1]      # [B, L_orig]
    ...
    def set_lambda_entropy(self, global_step: int):
        s0 = int(self.ponder_ent_warmup_steps)
        s1 = int(self.ponder_ent_peak_steps)
        a, b = float(self.ponder_ent_lambda_start), float(self.ponder_ent_lambda_max)
        if global_step <= s0:
            # 0 -> s0: a 很小的常数
            self.lambda_entropy = a
        elif global_step <= s1:
            # s0 -> s1: a → b 线性（你也可换 cosine）
            t = (global_step - s0) / max(1, (s1 - s0))
            self.lambda_entropy = a + (b - a) * t
        else:
            self.lambda_entropy = b

    def _compute_ponder_weights(self, hs_stage0: torch.Tensor):
        """
        hs_stage0: [B, L, H]  —— 来自阶段0（真实token对应阶段）的 hidden states
        返回:
        s: [B, L, K+1]         —— softmax 后的“额外步数”分布，s[...,0] 表示 0 步（直接用 E0）
        w: [B, L, K]         —— 逆向积分得到的权重（w_i = sum_{j=i..K} s_j）
        expected_steps: [B, L] —— 可选指标：期望步数 = sum_i i * s_i（从1计步）
        entropy: [B, L]      —— 分布熵
        """
        K = self.config.more_iterations
        if K == 0:
            device = hs_stage0.device
            s = hs_stage0.new_zeros(hs_stage0.size(0), hs_stage0.size(1), 0)
            w = s
            expected = hs_stage0.new_zeros(hs_stage0.size(0), hs_stage0.size(1))
            ent = hs_stage0.new_zeros_like(expected)
            return s, w, expected, ent

        logits_raw = self.ponder_head(hs_stage0)  # [B, L, K+1]
        if self.ponder_temp is not None and self.ponder_temp > 0:
            logits = logits_raw / self.ponder_temp

        s = torch.softmax(logits, dim=-1)  # [B, L, K+1], s[...,0] 是 0 步
        # 只对隐阶段(1..K) 计算 w：w_i = sum_{j=i..K} s_j
        s_hidden = s[..., 1:]                          # [B, L, K]
        # 逆向积分：w_i = s_i + s_{i+1} + ... + s_K
        # 用flip+cumsum实现
        w_rev = torch.flip(torch.cumsum(torch.flip(s_hidden, dims=[-1]), dim=-1), dims=[-1])  # [B, L, K]
        w = w_rev

        # 期望步数（从1开始）：E[step] = sum_i i * w_i
        idx = torch.arange(1, K + 1, device=hs_stage0.device, dtype=hs_stage0.dtype).view(1, 1, K)
        expected = torch.sum(w, dim=-1)  # [B, L]

        # 熵：H(s) = -sum s log s（避免log(0)）
        ent = -torch.sum(s * torch.clamp(torch.log(s + 1e-12), min=-50), dim=-1)  # [B, L]
        s_mean = s.mean(dim=(0,1))  # [K]
        ent_mean = -torch.sum(s_mean * torch.clamp(torch.log(s_mean + 1e-12), min=-50))  # [1]

        return s, w, expected, ent, ent_mean
    
    def _accumulate_ponder_losses(self, entropy, expected_steps, ent_mean):
        """
        把三类loss中的两类（熵、步数惩罚）聚合为标量；CE主损在forward末尾已有。
        返回：(ponder_aux_loss, entropy_mean, cost_mean)
        """
        # [B, L] -> 标量
        entropy_mean = entropy.mean()
        cost_mean = expected_steps.mean()  # 期望步数越小越好
        cost_loss_mode = getattr(self.config, "cost_loss_mode", "linear") # 'linear' | 'hinge' | 'quadratic'
        cost_target = float(getattr(self.config, "cost_target", 2))

        ponder_aux = 0.0
        entropy_loss = self.lambda_entropy * entropy_mean
        ponder_aux = ponder_aux + entropy_loss

        if cost_loss_mode == "linear":
            cost_loss = self.lambda_ponder_cost * cost_mean
        elif cost_loss_mode == "hinge":
            cost_loss = self.lambda_ponder_cost * torch.relu(cost_mean - cost_target)
        else:  # 'quadratic'
            cost_loss = self.lambda_ponder_cost * (cost_mean ** 2)
        ponder_aux = ponder_aux + cost_loss

        diverse_loss = - self.lambda_ent_w * ent_mean
        ponder_aux = ponder_aux + diverse_loss
        return ponder_aux, entropy_mean.detach(), entropy_loss.detach(), cost_mean.detach(), cost_loss.detach(), ent_mean.detach(), diverse_loss.detach()
    ...
    def forward(
        ...
        for iter_k_idx in range(num_initial_interpolation_stages): 
            ...
            iter_input_embeds, iter_pos_ids, iter_attn_mask, iter_ponder_w = self.prepare_gpt_inputs_for_stages(
                ...
                w_var=None if iter_k_idx == 0 else w_var   # 第0轮还没有 w_var
            )
            ...
            if iter_k_idx == 0:
                hs_E0 = self.extract_hidden_for_computation(
                    iter_gpt_neox_outputs, slice(0, None, num_stages_in_current_iter_input), self.config
                )  # [B, L, H]
                s_var, w_var, expected_steps_var, entropy_var, ent_w = self._compute_ponder_weights(hs_E0)
        ...
        if self.training:
            ...
            if getattr(self.config, "vary_refine_steps", False) and global_step is not None:
                num_refinement_steps = torch.randint(2, 5, (1,)).item()
        else:
        ...
        if num_initial_interpolation_stages > 0 and num_refinement_steps > 0: # Only refine if there are interpolated stages
            for ref_idx in range(num_refinement_steps):
                ...
                # Initialize with the last interpolated embedding *before* refinement starts
                # This will be compared against the result of the first refinement step.
                previous_refined_embedding_for_mse = None
                if num_initial_interpolation_stages > 0 and num_refinement_steps > 0: # Ensure there's an embedding to refine from
                    previous_refined_embedding_for_mse = all_embeddings_stages[1].detach().clone()
                ...
                s_var, w_var, expected_steps_var, entropy_var, ent_w = self._compute_ponder_weights(hs_E0_ref)
                # Refine each interpolated stage E_interp_0, ..., E_interp_{N}
                # N = num_initial_interpolation_stages - 1
                # E_interp_i (all_embeddings_stages[i+1]) is refined using hidden states from
                # E_interp_{i-1} (or E_orig if i=0), which corresponds to slice `i` of the input.
                ...
                for i in range(num_initial_interpolation_stages): # Loop K times, for E_interp_0 to E_interp_{K-1}
                    ...
                    # Calculate and print MSE change from the previous version of this embedding
                    previous_refined_embedding_for_mse = all_embeddings_stages[i+1].detach().clone()
                    # refined_embeds_for_stage = self._apply_ponder_weights_to_stages([all_embeddings_stages[0], refined_embeds_for_stage], w_var[..., i:i+1])[1]
                    if previous_refined_embedding_for_mse is not None:
                        ...
                        # print(f"Refinement Iteration {ref_idx + 1} interpolation step {i + 1}: MSE change for the refined embedding = {mse_change.item()}, Relative MSE = {relative_mse_change.item()}")
                ...
                log_payload = {
                    "refine/step": int(ref_idx),
                    "refine/mse_mean": float(sum(mse_series) / max(1, len(mse_series))),
                    "refine/rel_mse_mean": float(sum(rel_series) / max(1, len(rel_series))),
                    "refine/margin_mean": margin_mean,
                }
                w_stage_mean = w_var.mean(dim=(0,1))  # [K]
                K = w_var.shape[-1]
                for k in range(K):
                    log_payload[f"refine/w_stage_mean/{k+1}"] = float(w_stage_mean[k].detach().cpu())
                    w_k = w_var[..., k]
                    mask_1e_4 = (w_k < 1e-4)
                    ration_global_k = mask_1e_4.float().mean().item()
                    log_payload[f"refine/gate/{k+1}<1e-4"] = ration_global_k
        ...
        final_interleaved_input_embeds, final_interleaved_pos_ids, final_interleaved_attn_mask, final_ponder_w = \
            self.prepare_gpt_inputs_for_stages(
                final_stages_for_pass, B, L_orig, H, device,
                original_repeated_position_ids, original_attention_mask_2d,
                 embed_scale, self.config, w_var)
        
        # new position ids (按两倍final_interleaved_input_embeds的长度)
        # final_interleaved_pos_ids = torch.arange(0, final_interleaved_input_embeds.shape[1], device=device)
        # final_interleaved_pos_ids = final_interleaved_pos_ids.unsqueeze(0).expand(B, -1)
        final_pass_gpt_neox_outputs = self.gpt_neox(
            ...
            head_mask=head_mask, past_key_values=past_key_values, use_cache=use_cache, # User's cache preference
            output_attentions=output_attentions, 
            output_hidden_states=user_requested_output_hidden_states, # User's preference
            return_dict=True, cache_position=cache_position, # User's cache_position
            ponder_gate=final_ponder_w,    # <--- 新增
        )
        # ponder_aux_loss, ent_mean, ent_loss, cost, cost_loss = self._accumulate_ponder_losses(entropy_var, expected_steps_var)
        ponder_aux_loss, ent_mean, ent_loss, cost, cost_loss, ent_w_final, diverse_loss = self._accumulate_ponder_losses(entropy_var, expected_steps_var, ent_w)

        # 原来的 logits 计算，将被替换:
        # logits_all_final_sequence = self.embed_out(final_pass_gpt_neox_outputs.last_hidden_state)
            
        loss = None
        # final_output_logits 将存储计算得到的用于损失和模型输出的 logits
        # 其形状将是 (batch_size, L_orig, vocab_size)，仅包含目标阶段的 logits
        final_output_logits = None
        
        all_hidden_states_from_final_pass = final_pass_gpt_neox_outputs.last_hidden_state

        # 确定用于选择目标阶段隐状态/logits的起始索引
        logits_selector_start_index = num_initial_interpolation_stages # 这是 K
        if num_initial_interpolation_stages == 0 and num_total_stages_final == 1: # 只有 E_orig
                    logits_selector_start_index = 0

        if logits_selector_start_index < num_total_stages_final : # 确保索引对于切片有效
            # 从所有最终阶段的隐状态中提取目标阶段的隐状态
            # 形状: (batch_size, L_orig, hidden_size)
            ...
            for i in range(num_total_stages_final):  # i = 0..K
                # 取出第 i 个阶段对应的 hidden 切片（步长为 num_total_stages_final 的交错）
                hs_i = all_hidden_states_from_final_pass[:, i::num_total_stages_final, :]  # [B, L, H]
                wi  = s_var[..., i].unsqueeze(-1)                                          # [B, L, 1]
                hidden_states_final += hs_i * wi
            # for i in range(1, num_total_stages_final): 
            #     hidden_states_final +=  all_hidden_states_from_final_pass[:, i::num_total_stages_final, :] * s_var[..., i-1].unsqueeze(-1)

            # 优化点：仅对选定的目标阶段的隐状态应用输出嵌入层
            # 形状: (batch_size, L_orig, vocab_size)
            final_output_logits = self.embed_out(hidden_states_final)
            
            if labels is not None:
                # 确保维度匹配以进行损失计算 (L_orig)
                if final_output_logits.shape[1] == L_orig:
                    shift_logits = final_output_logits[:, :-1, :].contiguous()
                    shift_labels = labels[:, 1:].contiguous() # 假设 labels 对应于 L_orig
                    loss_fct = CrossEntropyLoss()
                    base_loss = loss_fct(shift_logits.view(-1, self.config.vocab_size), shift_labels.view(-1))
                if self.training:
                    loss = base_loss + ponder_aux_loss
                    # TODO: 用 wandb 记录原始 loss, ent_loss, cost_loss 
                    wandb.log({
                        "train/base_loss": base_loss.item(),
                        "train/aux_loss": ponder_aux_loss.item(),
                        "train/ent_loss": ent_loss,
                        "train/cost_loss": cost_loss,
                        "train/total_loss": loss.item(),
                        "train/ent_mean": ent_mean,
                        "train/mean_steps": cost,
                        "train/ent_w":ent_w_final,
                        "train/diverse_loss":diverse_loss,
                    }, commit=False)
                else:
                    loss = base_loss
        ...
        if not return_dict:
            # 注意：返回的 logits 现在是 final_output_logits (目标阶段的logits)
            # 其形状和含义已改变，不再是所有交错阶段的 logits。
