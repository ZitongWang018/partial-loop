...
class GPTNeoXPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        print("using modeling_gpt_neox_mlp")
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        # 添加映射矩阵：hidden_size x hidden_size
        self.hidden_mapping = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
        self.act = ACT2FN[config.hidden_act]       
        self.post_init()        
    ...
    def forward(
        ...
        if self.config.recurrent_model:
            ...
            for iteration in range(self.config.more_iterations):
                ...
                # 使用映射矩阵对interpolated_embeds进行变换
                interpolated_embeds = self.act(self.hidden_mapping(interpolated_embeds))
