...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        if hasattr(self.config, 'recurrent_model') and self.config.recurrent_model:
            ...
            # 重复 more_iteration+1 次通过所有layers
            more_iterations = getattr(self.config, 'more_iterations', 0)
            
            for repeat in range(more_iterations + 1):
                for i, layer in enumerate(self.layers):
                    if self.gradient_checkpointing and self.training and i < self.checkpoint_num_layers:
                        outputs = self._gradient_checkpointing_func(
                            ...
                            position_ids,
                            head_mask[i],
                            False,  # use_cache=False for simplicity
                            None,
                            output_attentions,
                            cache_position,
                            position_embeddings,
                        ...
                        outputs = layer(
                            ...
                            layer_past=None,  # no cache in recurrent mode
                            use_cache=False,
                            output_attentions=output_attentions,
                            cache_position=cache_position,
                            position_embeddings=position_embeddings,
