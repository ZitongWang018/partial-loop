...
class GPTNeoXPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        print("using modeling_gpt_neox_pausetoken")
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def _insert_pause_tokens(self, input_ids):
        """
        在每个token后插入指定数量的pause token
        Args:
            input_ids: 原始输入序列 [batch_size, seq_len]
        Returns:
            new_input_ids: 插入pause token后的序列
        """
        batch_size, seq_len = input_ids.shape
        device = input_ids.device
        
        # 获取pause token数量和ID
        num_pause_tokens = getattr(self.config, 'more_iterations', 0)
        pause_token_id = getattr(self.config, 'pause_token_id', 50288) # 默认值，应该在配置中指定

        if num_pause_tokens == 0:
            return input_ids

        # 新序列长度
        new_seq_len = seq_len * (1 + num_pause_tokens)
        
        # 创建新的input_ids
        new_input_ids = torch.full((batch_size, new_seq_len), pause_token_id, dtype=input_ids.dtype, device=device)
        
        # 填充原始tokens
        indices = torch.arange(seq_len, device=device) * (1 + num_pause_tokens)
        new_input_ids[:, indices] = input_ids
    ...
    def forward(
        ...
        # 对齐 LLaMA：当 position_ids 为 None 时，按当前输入长度构造
        if position_ids is None:
            if input_ids is not None:
                device = input_ids.device
        ...
        # 修复1：在训练和评估时都插入pause tokens，保持一致性
        if input_ids is not None and num_pause_tokens > 0:
            modified_input_ids = self._insert_pause_tokens(input_ids)

            if attention_mask is not None:
                attention_mask = attention_mask.repeat_interleave(1 + num_pause_tokens, dim=1)
            
            if position_ids is not None:
                position_ids = position_ids.repeat_interleave(1 + num_pause_tokens, dim=1)
        ...
        if orig_seq_len != -1 and num_pause_tokens > 0 and hidden_states.shape[1] > orig_seq_len:
            device = hidden_states.device
            # 修正评测逻辑 (需要配合修正后的训练逻辑重新训练模型):
            # 经过修正的训练后，模型学会了使用最后一个 pause token 的 hidden state 进行预测。
            # 最后一个 pause token 在原始 token j 之后的位置是 j*(1+n)+n。
            # 因此，我们选取这些位置的 hidden states 来计算 logits，不再需要 "- 1"。
            selected_indices = (torch.arange(orig_seq_len, device=device) * (1 + num_pause_tokens)) + num_pause_tokens

            max_len = hidden_states.shape[1]
            selected_indices = selected_indices[selected_indices < max_len]
        ...
        if labels is not None:
            # a long sentence might have a lot of -100, so we need to calculate the loss only for the non -100 tokens
