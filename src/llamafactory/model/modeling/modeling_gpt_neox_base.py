...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def _log_to_wandb(self, payload: Dict[str, float], commit: bool = False) -> None:
        if not payload:
            return
        ...
        try:
            ...
            print(f"[wandb.debug] logged keys={list(payload.keys())}", flush=True)
        except Exception as exc:
            print(f"[WARN] wandb.log failed ({exc}); keys={list(payload.keys())}")
            self._wandb_inited = False
    ...
    def forward(
        ...
        if not hasattr(self, "_wandb_inited"):
            self._wandb_inited = run is not None
        ...
        if self.training:
            ...
            if getattr(self.config, "vary_refine_steps", False) and global_step is not None:
                ...
                num_refinement_steps = torch.randint(4, 7, (1,)).item()
        ...
        if logits_selector_start_index < num_total_stages_final : # 确保索引对于切片有效
            ...
            if labels is not None:
                ...
                if final_output_logits.shape[1] == L_orig:
                    ...
                    self._log_to_wandb(
                        {
                            "train/base_loss": loss.detach().float().cpu().item(),
                        },
                        commit=False,
                    )
