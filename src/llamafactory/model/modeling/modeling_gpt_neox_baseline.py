...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # ---- Refinement Loop ----
        if self.training:
            num_refinement_steps = getattr(self.config, 'training_refinement_steps', 2)
            if getattr(self.config, "vary_refine_steps", False) and global_step is not None:
                ...
                #     # 使用简化的查表法来确定最大refine次数
                #     steps_since_start = global_step - start_step
                #     if steps_since_start < 200:
                #         max_steps = 3
                #     elif steps_since_start < 400:
                #         max_steps = 4
                #     elif steps_since_start < 600:
                #         max_steps = 5
                #     elif steps_since_start < 800:
                #         max_steps = 6
                #     elif steps_since_start < 1000:
                #         max_steps = 7
                #     elif steps_since_start < 1200:
                #         max_steps = 8
                #     elif steps_since_start < 1400:
                #         max_steps = 9
                #     else:  # >= 700
                #         max_steps = 10  # 修正：确保最终能达到10

                #     if 2 <= max_steps:
                #         num_refinement_steps = torch.randint(2, max_steps + 1, (1,)).item()
                        
                # # 在课程学习开始前，num_refinement_steps会保持其默认值(2)，无需额外处理
                num_refinement_steps = torch.randint(3, 6, (1,)).item()

                # # --- 关键：恢复RNG状态 ---
                # torch.set_rng_state(rng_state)

        else:
            num_refinement_steps = getattr(self.config, 'eval_refinement_steps', 2)
            # num_refinement_steps = torch.randint(3, 6, (1,)).item()            
