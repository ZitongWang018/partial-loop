...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if self.config.recurrent_model:
            ...
            # 迭代优化
            for iteration in range(self.config.more_iterations):
                # 判断是否需要计算梯度：只对最后back_iterations次迭代计算梯度
                should_compute_gradients = iteration >= (self.config.more_iterations - self.config.back_iterations)
                
                if should_compute_gradients:
                    # 对最后几次迭代计算梯度
                    ...
                    if self.config.output_hidden_states and self.config.hidden_layer_num:
                        interpolated_embeds = self.compute_interpolated_embeds(weight, outputs.hidden_states, use_topk=False)                
                    else:
                    ...
                    with torch.no_grad():
                        ...
                        if self.config.output_hidden_states and self.config.hidden_layer_num:
                            ...
                            interpolated_embeds = self.compute_interpolated_embeds(weight, outputs.last_hidden_state, use_topk=False)
                    # 注意：current_embeds的更新需要在no_grad外面，以确保梯度能正确传播
                    current_embeds = current_embeds + interpolated_embeds
