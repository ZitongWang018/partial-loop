...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # ---- START: Inference Path for Pondering (use_cache=True indicates inference) ----
        if past_key_values is not None:
            # Determine if this is decoding phase (past_key_values exists) or prefilling phase
            is_decoding = (len(past_key_values) > 0)
            
            if is_decoding:
                # --- Decoding Phase: Single token generation with pondering ---
                if inputs_embeds is None: 
                ...
                if hasattr(self.config, 'scale_embeds') and self.config.scale_embeds:
                    if self.config.interpolation == True:
                        embed_scale = torch.sqrt(torch.tensor(H, dtype=inputs_embeds.dtype, device=device))
                    else:
                        embed_scale = torch.sqrt(2.5*torch.tensor(H, dtype=inputs_embeds.dtype, device=device))
                    inputs_embeds0 = inputs_embeds * embed_scale
                    weight_for_interpolation = self.gpt_neox.embed_in.weight * embed_scale

                # First forward pass: get hidden state
                first_pass_outputs = self.gpt_neox(
                ...
                # Use the hidden state as pondered embedding
                pondered_embeds = first_pass_outputs.last_hidden_state
                updated_past_key_values = first_pass_outputs.past_key_values

                # Second forward pass: generate final output using pondered embedding
                second_pass_outputs = self.gpt_neox(
                ...
                logits = self.embed_out(second_pass_outputs.last_hidden_state)

                if not return_dict:
                    return (logits, second_pass_outputs.past_key_values)

                return CausalLMOutputWithPast(
                    loss=None,
                    logits=logits,
                    past_key_values=second_pass_outputs.past_key_values,
                    hidden_states=second_pass_outputs.hidden_states,
                    attentions=second_pass_outputs.attentions,
                )
            
            else:
                # --- Prefilling Phase: Process entire prompt with refinement ---
                if inputs_embeds is None:
                    if input_ids is None:
                        raise ValueError("input_ids must be provided")
                ...
                # Use the same refinement logic as training, but simplified
                ...
                # Initial Interpolation Loop (same as training)
                ...
                # Refinement Loop (simplified for inference)
                ...
                final_pass_outputs = self.gpt_neox(
                ...
                logits = self.embed_out(final_pass_outputs.last_hidden_state[:,-1:,:])
                
                if not return_dict:
                    return (logits, final_pass_outputs.past_key_values)

                return CausalLMOutputWithPast(
                    loss=None,
                    logits=logits,
                    past_key_values=final_pass_outputs.past_key_values,
                    hidden_states=final_pass_outputs.hidden_states,
                    attentions=final_pass_outputs.attentions,
                )
        # ---- END: Inference Path ----
        ...
        if self.training:
            ...
            if getattr(self.config, "vary_refine_steps", False):
                ...
                #     # 基于全局步数设置确定性种子，以同步所有进程
                #     torch.manual_seed(global_step)
                #     num_refinement_steps = torch.randint(2, 4, (1,)).item()
                    
                #     # 恢复之前的随机数生成器状态，确保不影响模型其他部分
                #     torch.set_rng_state(rng_state)
                # else:
                #     # 如果没有 global_step，则退回至原始的非同步随机（或者在这里实现广播）
                #     num_refinement_steps = torch.randint(2, 4, (1,)).item()
                # num_refinement_steps = torch.randint(2, 4, (1,)).item()
        ...
        if num_initial_interpolation_stages > 0 and num_refinement_steps > 0: # Only refine if there are interpolated stages
            for ref_idx in range(num_refinement_steps):
                ...
                for i in range(num_initial_interpolation_stages): # Loop K times, for E_interp_0 to E_interp_{K-1}
                    ...
                    #     )
                    #     print(f"Refinement Iteration {ref_idx + 1}: MSE change for the refined embedding = {mse_change.item()}, Relative MSE = {relative_mse_change.item()}")
                    
                    # Update the target interpolated stage all_embeddings_stages[i+1]
                    all_embeddings_stages[i+1] = refined_embeds_for_stage
                    previous_refined_embedding_for_mse = refined_embeds_for_stage.detach().clone()
        ...
        if logits_selector_start_index < num_total_stages_final : # 确保索引对于切片有效
            ...
            final_output_logits = self.embed_out(hidden_states_for_target_stage)
        ...
        if self.training and getattr(self.config, 'consistency_weight', 0) > 0 and num_initial_interpolation_stages > 0:
            ...
            hidden_for_consistency_check = self.extract_hidden_for_computation(
                final_pass_gpt_neox_outputs, consistency_source_selector, self.config
            )
