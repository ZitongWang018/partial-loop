...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        embed_out: Optional[torch.FloatTensor] = None,
        ...
        # 如果启用了递归层模式，准备嵌入层权重
        if hasattr(self.config, "recurrent_layer") and self.config.recurrent_layer:
            if hasattr(self.config, "scale_embeds") and self.config.scale_embeds:
                embed_scale = torch.sqrt(torch.tensor(inputs_embeds.shape[-1], dtype=inputs_embeds.dtype))
                scaled_embed_weight = self.embed_in.weight * embed_scale
            else:
                scaled_embed_weight = self.embed_in.weight
        ...
        # 递归层模式的参数
        recurrent_interval = getattr(self.config, "recurrent_interval", 0)
        ...
        for i, layer in enumerate(self.layers):
            ...
            # 递归层处理：每隔recurrent_interval层应用一次递归连接
            if (hasattr(self.config, "recurrent_layer") and self.config.recurrent_layer and 
                recurrent_interval > 0 and (i + 1) % recurrent_interval == 0 and (i+1) != len(self.layers)):
                # 计算logits和概率
                layer_logits = embed_out(hidden_states)
                layer_probs = torch.softmax(layer_logits, dim=-1)
                
                # 计算interpolated embeddings
                interpolated_embeds = torch.matmul(layer_probs, scaled_embed_weight)
                
                # 应用残差连接
                hidden_states = hidden_states + interpolated_embeds
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def normalize_hidden_states(self, hidden_states,target_std = 0.02):
        # 1. 首先进行标准化(均值为0,方差为1)
        mean = hidden_states.mean(dim=-1, keepdim=True)
        std = hidden_states.std(dim=-1, keepdim=True)
        normalized = (hidden_states - mean) / (std + 1e-3)  # 加上小值避免除0        
        # target_std = 1
    ...
    def forward(
        ...
        # 如果是递归模式，使用简化的逻辑
        if hasattr(self.config, "recurrent_model") and self.config.recurrent_model:
            # 获取初始embeddings
            if inputs_embeds is None:
            ...
            for iteration in range(self.config.more_iterations):
                ...
                if self.config.output_hidden_states:
                    hidden_states = outputs.hidden_states[self.config.hidden_layer_num]
                else:
                    hidden_states = outputs.last_hidden_state
                    
                # 计算所有位置的logits和probabilities
                logits = self.embed_out(hidden_states)
                probs = torch.softmax(logits, dim=-1)
                
                if hasattr(self.config, "scale_embeds") and self.config.scale_embeds:
                    interpolated_embeds = torch.matmul(probs, self.gpt_neox.embed_in.weight * embed_scale)
                else:
                    # 计算interpolated embeddings
                    interpolated_embeds = torch.matmul(probs, self.gpt_neox.embed_in.weight)
                
                if hasattr(self.config, "residual_interpolated_embeds") and self.config.residual_interpolated_embeds:
                    # 将interpolated embeddings作为残差加入
                    current_embeds = current_embeds + interpolated_embeds
                else:
                    current_embeds = inputs_embeds0 + interpolated_embeds
