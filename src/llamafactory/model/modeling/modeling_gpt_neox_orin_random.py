...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def compute_interpolated_embeds(self, weight: torch.Tensor, hidden: Union[torch.Tensor, Tuple[torch.Tensor]], use_topk: bool = True, global_step: Optional[int] = None) -> torch.Tensor:
        """
        ...
        Args:
            ...
            global_step (Optional[int]): 全局训练步数，用于动态调整温度参数。

        Returns:
            torch.Tensor: 插值后的 embedding，形状为 [batch_size, seq_len, hidden_size]
        """
        # 动态计算温度参数：从1.0逐渐降到0.1，然后保持不变
        if global_step is not None:
            # 假设在10000步内从1.0降到0.1，可以根据需要调整这个步数
            temperature_decay_steps = 12500
            if global_step >= temperature_decay_steps:
                temperature = 0.1
            else:
                # 线性衰减：temperature = 1.0 - 0.9 * (global_step / temperature_decay_steps)
                temperature = 1.0 - 0.9 * (global_step / temperature_decay_steps)
        # else:
        #     # 如果没有提供global_step，使用配置中的默认温度
        #     temperature = getattr(self.config, 'softmax_temperature', 1.0)
        ...
        if use_topk:
            ...
            if isinstance(hidden, torch.Tensor):
                logits = self.embed_out(hidden) / temperature
                probs = torch.softmax(logits, dim=-1)
            else:
                total = len(hidden)
                selected_layers = [hidden[i] for i in range(total - 1, 0, -self.config.hidden_layer_num)]
                probs = sum(
                    torch.softmax(self.embed_out(layer) / temperature, dim=-1)
                    for layer in selected_layers
                )
    ...
    def forward(
        ...
        if self.config.recurrent_model:
            ...
            if  self.config.scale_embeds:
                # embed_scale = torch.sqrt(2.5*torch.tensor(inputs_embeds0.shape[-1], dtype=inputs_embeds0.dtype))
            ...
            if self.config.mutiply_iterations:
                if isinstance(self.config.mutiply_iterations, bool):
                    current_embeds = inputs_embeds0 * self.config.more_iterations
                elif isinstance(self.config.mutiply_iterations, int):
                    current_embeds = inputs_embeds0 * self.config.mutiply_iterations
                else:
            ...
            # 根据训练/评估模式决定迭代次数
            if self.training:
                # 训练时：随机选择0到more_iterations之间的迭代次数
                rng_state = torch.get_rng_state()
                # 使用global_step作为种子，确保所有DDP进程在同一步使用相同的随机数
                torch.manual_seed(global_step)
                # --- 关键：恢复RNG状态 ---
                num_iterations = random.randint(0, self.config.more_iterations)
                torch.set_rng_state(rng_state)                               
            else:
                # 评估时：使用固定的more_iterations
                num_iterations = self.config.more_iterations
            
            # 迭代优化
            for iteration in range(num_iterations):
                # Forward pass获取hidden states
                ...
                if self.config.output_hidden_states and self.config.hidden_layer_num:
                    interpolated_embeds = self.compute_interpolated_embeds(weight, outputs.hidden_states, use_topk=False, global_step=global_step)                
                else:
                    interpolated_embeds = self.compute_interpolated_embeds(weight, outputs.last_hidden_state, use_topk=False, global_step=global_step)
