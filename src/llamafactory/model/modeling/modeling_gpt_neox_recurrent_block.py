...
from beartype.typing import Optional, List, Tuple as BeartypeTuple
...
# bias-less layernorm, adapted to use built-in LayerNorm
...
class Attend(nn.Module):
    ...
    def forward(self, q, k, v, mask = None, use_flash_attn = None):
        ...
        if use_flash_attn and q.is_cuda:
            return torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask = mask, is_causal=self.causal)
...
class StateContainer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.to_state_cross_attn = Attention(dim_head, use_flash_attn = use_flash_attn)

        self.state_self_attn = Attention(dim_head, use_flash_attn = use_flash_attn)
        self.from_state_cross_attn = Attention(dim_head, use_flash_attn = use_flash_attn)
    ...
    def read(self, x):
        assert exists(self.next_read_state)
        states, = self.next_read_state
        self.next_read_state = None

        normed_states = self.state_norm(states) + self.state_pos_ids
            ...
            for compress_factor in compress_factors:
                ...
                if compress_factor > 1:
                    compress_fn = nn.Sequential(
                        ...
                        nn.Conv1d(dim_head * 2, dim_head * 2, compress_factor, stride = compress_factor, groups = 2),
                        Rearrange('b d n -> b n d'),
                    )
                compress_fns.append(compress_fn)
            self.layers.append(compress_fns)

    def forward(self, past_memories: List[torch.Tensor], new_memories: List[torch.Tensor]):
        next_memories = []
        ...
        for past_memory, new_memory, compress_fns in zip_longest(past_memories, new_memories, self.layers):
            ...
            for mem_length, compress_factor, compress_fn in zip(self.mem_lengths, self.compress_factors, compress_fns):
                ...
                if (exists(new_memory) and not new_memory.shape[-2] == 0) and compress_factor > 1:
                    ...
                    new_memory_to_compress = new_memory[:, :, curtailed_slice]

                    if new_memory_to_compress.shape[-2] > 0:
                        new_memory_to_compress = rearrange(new_memory_to_compress, 'm b n d -> b n (m d)', m = 2)
                        new_memory_to_compress = compress_fn(new_memory_to_compress)
                        new_memory_to_compress = rearrange(new_memory_to_compress, 'b n (m d) -> m b n d', m = 2)
                        new_memory = new_memory_to_compress

                current_memory = safe_cat(current_memory, new_memory, dim = -2)
                
                if exists(current_memory):
                    new_memory, current_memory = (current_memory[:, :, :-mem_length], current_memory[:, :, -mem_length:])
                else:
                    new_memory = None

                next_memory_segments.insert(0, current_memory)

            next_memories.append(safe_cat(*next_memory_segments, dim=-2))
...
class GPTNeoXPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        print("using modeling_gpt_neox_addhidden")
...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        memories = torch.stack((key, value))
        ...
        # Reshape outputs but don't apply dense yet (will be done in GPTNeoXLayer)
...
class GPTNeoXLayer(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.is_recurrent_layer = (layer_idx + 1) in getattr(config, 'recurrent_layers', [])
        
        # 计算有多少state containers会读取
        num_external_state_reads = sum([
            int((layer_idx + 1) == read_layer) 
            for read_layer in getattr(config, 'read_recurrent_layers', [])
        ])
        
        num_state_reads = int(self.is_recurrent_layer) + num_external_state_reads
        
        # 调整dense层以适应拼接的state输出
        if num_state_reads > 0:
            self.attention.dense = nn.Linear(
                config.hidden_size * (1 + num_state_reads), 
                config.hidden_size, 
        ...
        if self.is_recurrent_layer:
            self.state_container = StateContainer(
                ...
                num_state_vectors=getattr(config, 'num_state_vectors', config.block_width),
    ...
    def forward(
        ...
        xl_memories: Optional[torch.Tensor] = None,
        read_from_state_containers: Optional[List] = None,
    ):
        ...
        attn_output = attention_layer_outputs[0]  # output_attn: attn_output, present, (attn_weights), memories
        
        outputs = list(attention_layer_outputs[1:]) # present, (attn_weights), memories
        xl_memories_out = outputs.pop(-1)

        # 读取state containers并拼接
        read_from_state_containers = default(read_from_state_containers, [])
        if self.is_recurrent_layer:
        ...
        for read_state_container in read_from_state_containers:
            to_state_out = read_state_container.read(normed_hidden_states)
            state_outputs.append(to_state_out)
        
        if state_outputs:
            attn_output = torch.cat([attn_output] + state_outputs, dim = -1)
        ...
        new_states = None
        if self.is_recurrent_layer:
            new_states = self.state_container.write(memories=xl_memories_out)
        ...
        final_outputs = [hidden_states] + outputs + [xl_memories_out, new_states]
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    def __init__(self, config):
        ...
        self.checkpoint_num_layers = getattr(config, 'checkpoint_num_layers', 0)

        # block recurrent specific
        if getattr(config, "recurrent_model", False):
            recurrent_layers = getattr(config, "recurrent_layers", [])
            read_recurrent_layers = getattr(config, "read_recurrent_layers", recurrent_layers)
            
            # 验证配置
            assert len(recurrent_layers) == len(read_recurrent_layers), \
                "recurrent_layers and read_recurrent_layers must have the same length"
            assert all(1 <= layer <= config.num_hidden_layers for layer in recurrent_layers), \
                "recurrent_layers must be in range [1, num_hidden_layers]"
            assert all(1 <= layer <= config.num_hidden_layers for layer in read_recurrent_layers), \
                "read_recurrent_layers must be in range [1, num_hidden_layers]"
            
            self.read_state_router = defaultdict(list)
            for write_layer, read_layer in zip(recurrent_layers, read_recurrent_layers):
                self.read_state_router[read_layer - 1].append(self.layers[write_layer - 1].state_container)

            self.mem_manager = MemoryManager(
                dim_head = config.hidden_size // config.num_attention_heads,
                layers = config.num_hidden_layers,
                mem_lengths = getattr(config, 'block_width', 512),
                compress_factors = 1
            )
    ...
    def forward(
        ...
        if getattr(self.config, "recurrent_model", False):
            ...
            input_blocks = inputs_embeds.split(block_width, dim=-2)
            
            output_hidden_states_list = []
            
            for i, hidden_states in enumerate(input_blocks):
                batch_size, seq_len, _ = hidden_states.shape
                
                iter_xl_memories = iter(xl_memories) if xl_memories is not None else iter([])
                iter_states = iter(states) if states is not None else iter([])
                next_xl_memories = []
                next_states = []

                for layer_idx, layer in enumerate(self.layers):
                    if layer.is_recurrent_layer:
                        layer.state_container.set_next_read_state(next(iter_states, None))

                position_ids = torch.arange(i * block_width, i * block_width + seq_len, device=hidden_states.device).unsqueeze(0)
                causal_mask = self._update_causal_mask(attention_mask, hidden_states, position_ids, past_key_values, output_attentions) # TODO: fix mask for blocks
                ...
                for layer_idx, layer in enumerate(self.layers):
                    layer_outputs = layer(
                        ...
                        layer_past=None, # cache not supported in recurrent mode for now
                        use_cache=False,
                        output_attentions=output_attentions,
                        cache_position=None,
                        position_embeddings=position_embeddings,
                        xl_memories=next(iter_xl_memories, None),
                        read_from_state_containers=self.read_state_router.get(layer_idx, [])
                    )

                    hidden_states = layer_outputs[0]
                    layer_xl_memories = layer_outputs[-2]
                    layer_next_states = layer_outputs[-1]
                ...
                states = next_states
                if seq_len == block_width and xl_memories is not None:
                    xl_memories = self.mem_manager(xl_memories, next_xl_memories)

            hidden_states = torch.cat(output_hidden_states_list, dim=1)
            hidden_states = self.final_layer_norm(hidden_states)

            if not return_dict:
                return (hidden_states, xl_memories, states)
            
            return BaseModelOutputWithPast(
                last_hidden_state=hidden_states,
                past_key_values=(xl_memories, states) # using this to pass memories and states
            )
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel, GenerationMixin):
    ...
    def compute_interpolated_embeds(self, weight: torch.Tensor, hidden: Union[torch.Tensor, Tuple[torch.Tensor]], use_topk: bool = True) -> torch.Tensor:
        """
        根据隐藏层输出计算插值后的 embedding。

        当 use_topk 为 True 时：
            如果 hidden 为单个 tensor，则只使用最后一层的 hidden state，取 top_k 进行加权；
            如果 hidden 为 tuple，则按照配置参数选取多个层（例如倒数每隔 hidden_layer_num 的一层）进行 top_k 计算，
            最后将各层结果取平均得到插值后的 embedding。
        当 use_topk 为 False 时，则根据配置参数选取多层 hidden state，
        并对所有选中的层计算 softmax 后求和，最后根据全局概率分布计算插值后的 embedding。

        Args:
            weight (torch.Tensor): token embedding 权重，形状为 [vocab_size, hidden_size]
            hidden (Union[torch.Tensor, Tuple[torch.Tensor]]): 若为 tensor，则代表最后一层的 hidden state，
                否则为包含所有层输出的 tuple，假设 hidden[0] 为输入嵌入，后续依次为每一层输出;
            use_topk (bool): 是否使用 top_k 策略，默认为 True。

        Returns:
            torch.Tensor: 插值后的 embedding，形状为 [batch_size, seq_len, hidden_size]
        """
        if use_topk:
            if isinstance(hidden, torch.Tensor):
                # 单层 topk 计算
                logits = self.embed_out(hidden)  # [B, L, vocab_size]
                logits_scaled = logits / self.config.softmax_temperature
                top_k = 100
                topk_values, topk_indices = torch.topk(logits_scaled, k=top_k, dim=-1)
                probs_topk = torch.softmax(topk_values, dim=-1)  # [B, L, top_k]
                # 根据 topk 索引从 weight 中提取对应的 token embedding，形状：[B, L, top_k, hidden_size]
                embedding_topk = weight[topk_indices]
                # 根据 softmax 概率加权求和得到插值后的 embedding，[B, L, hidden_size]
                interpolated_embeds = torch.einsum("bsl,bslh->bsh", probs_topk, embedding_topk)
                return interpolated_embeds
            else:
                # 多层 topk 计算：选取倒数若干层进行计算，并对每层结果取平均
                total = len(hidden)
                selected_layers = [hidden[i] for i in range(total - 1, 0, -self.config.hidden_layer_num)]
                list_interpolated = []
                top_k = 100
                for layer in selected_layers:
                    logits = self.embed_out(layer)  # [B, L, vocab_size]
                    logits_scaled = logits / self.config.softmax_temperature
                    topk_values, topk_indices = torch.topk(logits_scaled, k=top_k, dim=-1)
                    probs_topk = torch.softmax(topk_values, dim=-1)  # [B, L, top_k]
                    embedding_topk = weight[topk_indices]  # [B, L, top_k, hidden_size]
                    interpolated_layer = torch.einsum("bsl,bslh->bsh", probs_topk, embedding_topk)
                    list_interpolated.append(interpolated_layer)
                # 对各层计算结果取平均
                final_interpolated = sum(list_interpolated) 
                return final_interpolated
        else:
            # 不使用 topk 的计算方式：使用全层 softmax 概率
            if isinstance(hidden, torch.Tensor):
                logits = self.embed_out(hidden) / self.config.softmax_temperature
                probs = torch.softmax(logits, dim=-1)
            else:
                total = len(hidden)
                selected_layers = [hidden[i] for i in range(total - 1, 0, -self.config.hidden_layer_num)]
                probs = sum(
                    torch.softmax(self.embed_out(layer) / self.config.softmax_temperature, dim=-1)
                    for layer in selected_layers
                )
    ...
    def forward(
        ...
        input_ids: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        high_entropy_mask: Optional[torch.LongTensor] = None,  # 可以为None
        position_ids: Optional[torch.LongTensor] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        head_mask: Optional[torch.FloatTensor] = None,
        past_key_values: Optional[Union[Cache, Tuple[Tuple[torch.FloatTensor]]]] = None,
        ...
        xl_memories = states = None
        if self.config.recurrent_model:
            if past_key_values is not None:
                 xl_memories, states = past_key_values
