...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def _prepare_inputs_for_stages(
        ...
        scale_emb_val: Optional[torch.Tensor], # Not directly used in Llama version like in NeoX, but kept for signature consistency if needed later
        config_obj # self.config
    ):
        num_stages_to_interleave = len(current_stages_list)
        interleaved_len = num_stages_to_interleave * seq_len_orig

        # Initialize tensors for interleaved inputs
        # Note: In Llama, past_key_values are handled by the model directly, so we don't prepare them here.
        # We only prepare inputs_embeds, position_ids, and attention_mask for the model's forward pass.
        ...
        # LlamaModel typically expects a 4D attention mask (batch_size, 1, seq_len, seq_len) or 2D (batch_size, seq_len)
        # For interleaving, we construct a 2D mask first, LlamaModel's prepare_attention_mask will handle conversion if needed.
        ...
        for stage_idx in range(num_stages_to_interleave):
            ...
            loop_input_embeds_final[:, s_start::num_stages_to_interleave, :] = current_stages_list[stage_idx]
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        # Extract global_step from loss_kwargs if provided
        global_step = loss_kwargs.get("global_step", None)
        ...
        if getattr(self.config, "recurrent_model", False):
            ...
            for iter_k_idx in range(num_initial_interpolation_stages):
                ...
                iter_input_embeds, iter_pos_ids, iter_attn_mask = self._prepare_inputs_for_stages(
            ...
            if num_initial_interpolation_stages > 0 and num_refinement_steps > 0:
                for ref_idx in range(num_refinement_steps):
                    ...
                    ref_input_embeds, ref_pos_ids, ref_attn_mask = self._prepare_inputs_for_stages(
                    ...
                    interp_use_topk = getattr(self.config, 'interpolation_use_topk', False) # Config can be shared

                    for i in range(num_initial_interpolation_stages): # K stages to refine E_interp_0 to E_interp_{K-1}
                        ...
                        all_embeddings_stages[i+1] = refined_embeds_for_stage # Update E_interp_i
            ...
            # Determine target stage for logits (same logic as GPT-NeoX)
            logits_selector_start_index = num_initial_interpolation_stages 
            if num_initial_interpolation_stages == 0 and num_total_stages_final == 1: # Only E_orig
            ...
            if logits_selector_start_index < num_total_stages_final :
                ...
                final_output_logits = self.lm_head(hidden_states_for_target_stage) # Shape (B, L_orig, V)
            else: # Should not happen if logic is correct, but as a fallback:
                if num_total_stages_final > 0 :
                    hidden_states_for_target_stage = all_hidden_states_from_final_pass[:, 0::num_total_stages_final, :]
                    final_output_logits = self.lm_head(hidden_states_for_target_stage)
                else: # This is a critical error, no stages to process.
                    raise ValueError("No stages available in final_pass_model_outputs for recurrent model.")
            ...
            if labels is not None:
                ...
                loss = self.loss_function(logits=final_output_logits, labels=labels, vocab_size=self.config.vocab_size, **loss_kwargs)
            ...
            if self.training and getattr(self.config, 'consistency_weight', 0) > 0 and num_initial_interpolation_stages > 0:
                ...
                hidden_for_consistency_check = self._extract_hidden_for_computation(
                    final_pass_model_outputs, consistency_source_selector, self.config
                )

                # 使用 final pass 的隐状态重新计算 embedding
                interp_use_topk = getattr(self.config, 'interpolation_use_topk', False)
                if self.config.interpolation == True:
                    recomputed_embedding = self._compute_interpolated_embeds(
