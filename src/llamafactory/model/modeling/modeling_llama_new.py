...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def _router_scores_from_hs_stack(self, hs_stack: torch.Tensor, S: int) -> torch.Tensor:
        """
        Efficient diagonal scoring without computing full (K+1)-dim logits.
        hs_stack: [B, L, S, H]
        returns scores: [B, L, S]
        scores[..., i] = <hs_stack[..., i, :], W[i, :]> + b[i]
        where W,b are from self.ponder_head (Linear(H -> K+1)).
        """
        # ponder_head.weight: [K+1, H], bias: [K+1]
        W = self.ponder_head.weight[:S]  # [S, H]
        b = self.ponder_head.bias[:S] if (self.ponder_head.bias is not None) else None  # [S] or None

        # einsum: (B,L,S,H) dot (S,H) -> (B,L,S)
        scores = torch.einsum("blsh,sh->bls", hs_stack, W)
        if b is not None:
            scores = scores + b.view(1, 1, S)
        return scores


    def _compute_s_w_from_hs_list(self, hs_list, grow_next: bool = False):
        """
        hs_list: list length S_cur, each [B, L, H]
        If grow_next=True: output length S_cur+1 by predicting the new stage score
        using the last stage hidden and the (S_cur)-th row of ponder_head.
        Returns:
        s: [B, L, S_out]  (softmax normalized)
        w: [B, L, S_out]  (tail-CDF, monotone decreasing)
        expected: [B, L]
        ent: [B, L]
        ent_mean: scalar
        """
        assert isinstance(hs_list, (list, tuple)) and len(hs_list) > 0
        S_cur = len(hs_list)
        S_out = S_cur + (1 if grow_next else 0)

        Kp1 = int(self.config.more_iterations) + 1
        if S_out > Kp1:
            raise RuntimeError(f"S_out={S_out} exceeds head dim (K+1)={Kp1}. Check S_max==K+1.")

        # stack -> [B, L, S_cur, H]
        hs_stack = torch.stack(hs_list, dim=2)

        # scores for existing stages: [B, L, S_cur]
        scores_exist = self._router_scores_from_hs_stack(hs_stack, S_cur)

        if grow_next:
            # score for new stage index = S_cur, using last stage hidden as proxy:
            Wn = self.ponder_head.weight[S_cur]  # [H]
            bn = self.ponder_head.bias[S_cur] if (self.ponder_head.bias is not None) else None
            hs_last = hs_list[-1]  # [B,L,H]
            score_new = torch.einsum("blh,h->bl", hs_last, Wn)
            if bn is not None:
                score_new = score_new + bn
            scores = torch.cat([scores_exist, score_new.unsqueeze(-1)], dim=-1)  # [B,L,S_out]
        else:
            scores = scores_exist  # [B,L,S_cur]

        s = torch.softmax(scores, dim=-1)  # [B,L,S_out]
        w = torch.flip(torch.cumsum(torch.flip(s, dims=[-1]), dim=-1), dims=[-1])  # [B,L,S_out]

        idx = torch.arange(0, S_out, device=s.device, dtype=s.dtype).view(1, 1, S_out)
        expected = torch.sum(idx * s, dim=-1)  # [B,L]

        ent = -torch.sum(s * torch.clamp(torch.log(s + 1e-12), min=-50), dim=-1)  # [B,L]
        s_mean = s.mean(dim=(0, 1))  # [S_out]
        ent_mean = -torch.sum(s_mean * torch.clamp(torch.log(s_mean + 1e-12), min=-50))

        return s, w, expected, ent, ent_mean

    def _compute_s_w_from_model_outputs(self, model_outputs, S: int, grow_next: bool = False):
        """
        Extract hs_list from interleaved last_hidden_state and compute s/w efficiently.
        last_hidden: [B, L*S, H]
        hs_i = last_hidden[:, i::S, :]
        """
        last_hidden = model_outputs.last_hidden_state if hasattr(model_outputs, "last_hidden_state") else model_outputs[0]
        B, LS, H = last_hidden.shape
        if LS % S != 0:
            raise RuntimeError(f"Interleaved length {LS} not divisible by S={S}")
        hs_list = [last_hidden[:, i::S, :] for i in range(S)]
        return self._compute_s_w_from_hs_list(hs_list, grow_next=grow_next)
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if not hasattr(self, "_wandb_inited"):
            # Do not initialize W&B inside model forward.
            # Trainer/WandbCallback should own lifecycle to avoid multi-rank duplicated runs.
            self._wandb_inited = wandb.run is not None
            self._wandb_refine_defined = False
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        disable_model_wandb_log = os.getenv("LLAMA_DISABLE_MODEL_WANDB_LOG", "0").strip().lower() in {"1", "true", "yes"}
        if self.training:
            if global_step is None:
        ...
        if getattr(self.config, "recurrent_model", False):
            ...
            num_initial_interpolation_stages = getattr(self.config, 'more_iterations', 0)  # K
            K = num_initial_interpolation_stages
            ...
            # ======= Pre-forward = iter0 (S=1), used to create E1 and compute gate for S=2 =======
            if K > 0:
                pre_iter_stages = [all_embeddings_stages[0]]  # [E0], S=1
                pre_iter_input_embeds, pre_iter_pos_ids, pre_iter_attn_mask, pre_iter_ponder_w = self._prepare_inputs_for_stages(
                    ...
                    w_var=None  # S=1: gate defaults to ones
                )
                ...
                pre_iter_model_outputs = self.model(
                    ...
                    ponder_gate=pre_iter_ponder_w,
                )

                # stage0 hidden (S=1)
                hs_E0_pre = self._extract_hidden_for_computation(pre_iter_model_outputs, slice(0, None, 1), self.config)  # [B,L,H]

                # create first interpolated stage E1 from hs_E0_pre
                interp_use_topk = getattr(self.config, 'interpolation_use_topk', False)
                if self.config.interpolation == True:
                    E1 = self._compute_interpolated_embeds(weight_for_interpolation, hs_E0_pre, use_topk=interp_use_topk)
                else:
                    E1 = hs_E0_pre

                all_embeddings_stages.append(E1)  # now S=2 (E0,E1)

                # compute s/w for NEXT pass with S=2 so first S=2 forward is gated
                s_var, w_var, expected_steps_var, entropy_var, ent_mean_var = \
                    self._compute_s_w_from_hs_list([hs_E0_pre], grow_next=True)
            else:
                # K==0: degenerate single stage
                s_var = initial_embeds_raw.new_ones(B, L_orig, 1)
                w_var = initial_embeds_raw.new_ones(B, L_orig, 1)
                expected_steps_var = initial_embeds_raw.new_zeros(B, L_orig)
                entropy_var = initial_embeds_raw.new_zeros(B, L_orig)
                ent_mean_var = initial_embeds_raw.new_zeros(())
            ...
            # ---- Initial Interpolation Loop (iter=1..K-1) ----
            for iter_k_idx in range(1, K):
                current_stages_for_iter_pass = list(all_embeddings_stages)
                S_cur = len(current_stages_for_iter_pass)  # should be iter_k_idx+1
                ...
                iter_input_embeds, iter_pos_ids, iter_attn_mask, iter_ponder_w = self._prepare_inputs_for_stages(
                    ...
                    original_repeated_position_ids, original_attention_mask_2d,
                    embed_scale, self.config,
                    w_var=w_var  # IMPORTANT: this gates the S_cur interleaved slots
                )
                ...
                iter_model_outputs = self.model(
                    ...
                    return_dict=True, cache_position=None,
                    ponder_gate=iter_ponder_w,
                )

                interp_use_topk = getattr(self.config, 'interpolation_use_topk', False)

                # 1) extract per-stage hidden slices and compute embeddings for refinement/append
                hs_slices_for_router = []
                computed_embeddings_from_hs_slices = []

                for stage_slice_idx in range(S_cur):
                    selector = slice(stage_slice_idx, None, S_cur)
                    hs_i = self._extract_hidden_for_computation(iter_model_outputs, selector, self.config)  # [B,L,H]
                    hs_slices_for_router.append(hs_i)

                    if self.config.interpolation == True:
                        emb_i = self._compute_interpolated_embeds(weight_for_interpolation, hs_i, use_topk=interp_use_topk)
                    else:
                        emb_i = hs_i
                    computed_embeddings_from_hs_slices.append(emb_i)

                # 2) refine existing latent stages E1..E_{iter_k_idx}  (count = iter_k_idx)
                for j in range(iter_k_idx):
                    all_embeddings_stages[j + 1] = computed_embeddings_from_hs_slices[j]

                # 3) append new stage E_{iter_k_idx+1}
                all_embeddings_stages.append(computed_embeddings_from_hs_slices[iter_k_idx])

                # 4) update s/w for NEXT pass (S_cur -> S_cur+1), no extra forward
                s_var, w_var, expected_steps_var, entropy_var, ent_mean_var = \
                    self._compute_s_w_from_hs_list(hs_slices_for_router, grow_next=True)
            ...
            if num_initial_interpolation_stages > 0 and num_refinement_steps > 0:
                ...
                if getattr(self, "_wandb_refine_defined", False) is False:
                    try:
                        ...
                        if is_rank0 and (wandb.run is not None) and (not disable_model_wandb_log):
                ...
                for ref_idx in range(num_refinement_steps):
                    ...
                    # ---- update w/s (multi-stage, efficient) ----
                    s_new, w_new, expected_steps_new, entropy_new, ent_mean_new = \
                        self._compute_s_w_from_model_outputs(ref_model_outputs, S_ref, grow_next=False)
                ...
                if self.training:
                    ...
                    try:
                        ...
                        if step_accuracies is not None:
                            for i, acc in enumerate(step_accuracies):
                                log_dict[f"train/step_accuracy_w{i}"] = float(acc.detach().cpu())
                        if (wandb.run is not None) and (not disable_model_wandb_log):
                            wandb.log(log_dict, commit=False)
                    except Exception:
                        pass
                else:
