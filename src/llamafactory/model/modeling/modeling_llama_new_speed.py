...
from .flash_attention_vecbias import can_use_flash_attn_vecbias, flash_attn_vecbias
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        d_orig = query_states.size(-1)
        softmax_scale = 1.0 / math.sqrt(d_orig)

        used_vecbias_kernel = False
        ...
        if ponder_gate is not None:
            ...
            if can_use_flash_attn_vecbias(
                ...
                attn_output = flash_attn_vecbias(
                    query_states,
                    key_states,
                    value_states,
                    G_hard,
                ...
                used_vecbias_kernel = True
            else:
                G_exp = G_hard.unsqueeze(2).expand(B, L, H_k)

                query_states = F.pad(query_states, (0, pad_w), value=0)
                query_states[..., d_orig] = math.sqrt(float(d_aug))
                key_states = F.pad(key_states, (0, pad_w), value=0)
                key_states[..., d_orig] = G_exp
                value_states = F.pad(value_states, (0, pad_w), value=0)

        if not used_vecbias_kernel:
            attn_output = _flash_attention_forward(
                query_states,
                key_states,
                value_states,
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def _prepare_inputs_for_stages(
        ...
        loop_attn_mask = None if mask_is_all_ones else torch.empty((batch_size, interleaved_len), dtype=orig_attn_mask_2d.dtype, device=dev)
        loop_input_embeds_final = torch.empty((batch_size, interleaved_len, hidden_size), dtype=current_stages_list[0].dtype, device=dev)
        ...
        if do_prune and loop_attn_mask is None:
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if not hasattr(self, "_wandb_inited"):
            try:
                if is_rank0 and os.getenv("WANDB_DISABLED", "").lower() not in {"1", "true", "yes", "on"} and wandb.run is None:
                    wandb.init(project="Ponder2-A", reinit=False)
                self._wandb_inited = is_rank0
                # wandb.define_metric("refine/step")  # x 轴
                # wandb.define_metric("refine/*", step_metric="refine/step")
                self._wandb_refine_defined = not is_rank0
        ...
        if getattr(self.config, "recurrent_model", False):
            ...
            disable_ponder_gate_ablation = os.getenv("LLAMAFACTORY_ABLATE_PONDER_GATE", "").lower() in {"1", "true", "yes", "on"}
            disable_refine_weight_update_ablation = os.getenv("LLAMAFACTORY_ABLATE_REFINE_WEIGHT_UPDATE", "").lower() in {"1", "true", "yes", "on"}
            disable_step_penalty_compute_ablation = os.getenv("LLAMAFACTORY_ABLATE_STEP_PENALTY_COMPUTE", "").lower() in {"1", "true", "yes", "on"}
            detach_ponder_gate_grad_ablation = os.getenv("LLAMAFACTORY_ABLATE_DETACH_PONDER_GATE_GRAD", "").lower() in {"1", "true", "yes", "on"}
            if inputs_embeds is None:
            ...
            original_attention_mask_all_ones = bool(torch.all(original_attention_mask_2d != 0).item())
            ...
            if num_initial_interpolation_stages > 0:
                ...
                pre_iter_input_embeds, pre_iter_pos_ids, pre_iter_attn_mask, pre_iter_ponder_w = self._prepare_inputs_for_stages(
                    ...
                    mask_is_all_ones=original_attention_mask_all_ones,
                )
                ...
                pre_iter_model_outputs = self.model(
                    ...
                    ponder_gate=None if disable_ponder_gate_ablation else (pre_iter_ponder_w.detach() if detach_ponder_gate_grad_ablation else pre_iter_ponder_w),
                )
            ...
            # 第0轮只有 stage0 时，ponder_gate 仍然恒为 1，可直接复用 pre_iter 的输出。
            for iter_k_idx in range(num_initial_interpolation_stages):
                ...
                if iter_k_idx == 0 and num_stages_in_current_iter_input == 1:
                    iter_model_outputs = pre_iter_model_outputs
                else:
                    ...
                    iter_model_outputs = self.model(
                        ...
                        return_dict=True, cache_position=None,
                        ponder_gate=None if disable_ponder_gate_ablation else (iter_ponder_w.detach() if detach_ponder_gate_grad_ablation else iter_ponder_w),
                    )
            ...
            if num_initial_interpolation_stages > 0 and num_refinement_steps > 0:
                ...
                # Refinement method: "jacobi" (default) = x_new = G(x); "anderson" = Anderson acceleration
                use_anderson = bool(getattr(self.config, "use_anderson_acceleration", False))
                anderson_ridge = float(getattr(self.config, "anderson_ridge", 1e-5))
                anderson_prev_embed: List[Optional[torch.Tensor]] = [None] * num_initial_interpolation_stages if use_anderson else []
                anderson_prev_refined: List[Optional[torch.Tensor]] = [None] * num_initial_interpolation_stages if use_anderson else []
                ...
                for ref_idx in range(num_refinement_steps):
                    ...
                    ref_model_outputs = self.model(
                        ...
                        return_dict=True, cache_position=None,
                        ponder_gate=None if disable_ponder_gate_ablation else (ref_ponder_w.detach() if detach_ponder_gate_grad_ablation else ref_ponder_w),
                    )

                    # ---- update w/s (same behavior) ----
                    if not disable_refine_weight_update_ablation:
                    ...
                    for i in range(num_initial_interpolation_stages):
                        ...
                        # Refinement update: Jacobi (default) x_new = G(x) vs Anderson acceleration
                        x_curr = all_embeddings_stages[i + 1]
                        if not use_anderson:
                            # Jacobi iteration: x_new = G(x_curr)
                            all_embeddings_stages[i + 1] = refined_embeds_for_stage
                        elif anderson_prev_refined[i] is None:
                            # First refinement step with Anderson: simple fixed-point update
                            all_embeddings_stages[i + 1] = refined_embeds_for_stage
                            anderson_prev_embed[i] = x_curr.detach()
                            anderson_prev_refined[i] = refined_embeds_for_stage.detach()
                        else:
                            # Anderson acceleration: x_new = (1-theta)*G(x_curr) + theta*G(x_prev)
                            # theta minimizes ||(1-theta)*F_curr + theta*F_prev||^2, F = G(x) - x
                            # Minimizing ||F_curr + theta*delta_f||^2 => theta = -(F_curr·delta_f)/(delta_f·delta_f)
                            F_curr = refined_embeds_for_stage - x_curr
                            F_prev = anderson_prev_refined[i] - anderson_prev_embed[i]
                            delta_f = F_prev - F_curr
                            delta_f_flat = delta_f.reshape(-1).float()
                            F_curr_flat = F_curr.reshape(-1).float()
                            denom = (delta_f_flat * delta_f_flat).sum() + anderson_ridge
                            theta = -(F_curr_flat * delta_f_flat).sum() / denom.clamp_min(1e-12)
                            theta = theta.clamp(0.0, 1.0).to(refined_embeds_for_stage.dtype)
                            x_new = (1.0 - theta) * refined_embeds_for_stage + theta * anderson_prev_refined[i]
                            all_embeddings_stages[i + 1] = x_new
                            anderson_prev_embed[i] = x_curr.detach()
                            anderson_prev_refined[i] = refined_embeds_for_stage.detach()
            ...
            final_interleaved_input_embeds, final_interleaved_pos_ids, final_interleaved_attn_mask, final_gate = \
                self._prepare_inputs_for_stages(
                    ...
                    original_repeated_position_ids, original_attention_mask_2d,
                    embed_scale, self.config, w_var=w_var, mask_is_all_ones=original_attention_mask_all_ones, force_eval_prune=False)
            ...
            final_pass_model_outputs = self.model(
                ...
                ponder_gate=None if disable_ponder_gate_ablation else (final_gate.detach() if detach_ponder_gate_grad_ablation else final_gate),
            )
            
            all_hidden_states_from_final_pass = final_pass_model_outputs.last_hidden_state
            if s_var is None or S == 1:
                hidden_states_final = all_hidden_states_from_final_pass[:, 0::S, :]
            else:
            ...
            if (
                ...
                and w_var is not None
                and S > 0
                and not disable_step_penalty_compute_ablation
            ):
                if final_output_logits.shape[1] == L_orig:
                    ...
                    need_step_accuracy_logging = is_rank0 and (wandb.run is not None)
                    step_accuracies = [] if need_step_accuracy_logging else None
                    # 根据方法类型分别计算
                    if self.min_weight_penalty_method == "accuracy":
                        # 方法1：基于softmax概率计算惩罚比例
                        if step_accuracies is None:
                            step_accuracies = []
                        step_penalty_ratios = []
                        ...
                        valid_token_count = mask.float().sum()
                        has_valid_tokens = bool((valid_token_count > 0).item())

                        with torch.no_grad():
                            batched_path_ok = all_hidden_states_from_final_pass.shape[1] == (L_orig * S)

                            if batched_path_ok:
                                hidden_size = all_hidden_states_from_final_pass.shape[-1]
                                stage_hidden_states = all_hidden_states_from_final_pass.reshape(B, L_orig, S, hidden_size)
                                weighted_stage_hidden_states = stage_hidden_states * s_var.unsqueeze(-1)
                                cumulative_hidden_states = torch.cumsum(weighted_stage_hidden_states, dim=2)

                                if S > 1:
                                    prefix_hidden_states = cumulative_hidden_states[:, :, :-1, :].reshape(
                                        B * L_orig * (S - 1), hidden_size
                                    )
                                    prefix_logits = self.lm_head(prefix_hidden_states).reshape(
                                        B, L_orig, S - 1, self.config.vocab_size
                                    )
                                    logits_all_steps = torch.cat(
                                        (prefix_logits, final_output_logits.detach().unsqueeze(2)), dim=2
                                    )
                                else:
                                    logits_all_steps = final_output_logits.detach().unsqueeze(2)

                                shift_logits_all = logits_all_steps[:, :-1, :, :].contiguous()  # [B, L-1, S, vocab_size]

                                if need_step_accuracy_logging:
                                    step_preds = torch.argmax(shift_logits_all, dim=-1)  # [B, L-1, S]
                                    if has_valid_tokens:
                                        correct = (step_preds == shift_labels.unsqueeze(-1)) & mask.unsqueeze(-1)
                                        step_accuracies = (correct.float().sum(dim=(0, 1)) / valid_token_count).detach()
                                    else:
                                        step_accuracies = torch.zeros(S, device=device)

                                # w0 不惩罚，只需要计算 w1..wK 对应的 CE penalty ratio
                                if S > 1:
                                    penalty_logits = shift_logits_all[:, :, 1:, :].contiguous()  # [B, L-1, S-1, vocab_size]
                                    expanded_labels = shift_labels.unsqueeze(-1).expand(-1, -1, S - 1)
                                    per_token_ce = loss_fct(
                                        penalty_logits.reshape(-1, self.config.vocab_size),
                                        expanded_labels.reshape(-1),
                                    ).reshape(B, shift_logits_all.shape[1], S - 1)

                                    if has_valid_tokens:
                                        per_token_score = torch.sigmoid(10.0 * (per_token_ce - 0.5))
                                        masked_scores = per_token_score * mask.unsqueeze(-1).to(per_token_score.dtype)
                                        mean_score = masked_scores.sum(dim=(0, 1)) / valid_token_count
                                        step_penalty_ratios = (1.0 - mean_score).detach()
                                    else:
                                        step_penalty_ratios = torch.zeros(S - 1, device=device)
                            else:
                                hs_step_i = None
                                for i in range(0, S):  # i=0..K
                                    stage_hidden = all_hidden_states_from_final_pass[:, i::S, :] * s_var[..., i].unsqueeze(-1)
                                    hs_step_i = stage_hidden if hs_step_i is None else (hs_step_i + stage_hidden)

                                    logits_step_i = final_output_logits.detach() if i == S - 1 else self.lm_head(hs_step_i)
                                    ...
                                    if logits_step_i.shape[1] == L_orig:
                                        ...
                                        if need_step_accuracy_logging:
                                            ...
                                            if has_valid_tokens:
                                                ...
                                                accuracy = correct.float().sum() / valid_token_count
                                                step_accuracies.append(accuracy.detach())
                                            else:
                                                step_accuracies.append(torch.tensor(0.0, device=device))

                                        # w0 不惩罚，不需要计算它的 CE penalty ratio
                                        if i >= 1:
                                            per_token_ce = loss_fct(
                                            ...
                                            if has_valid_tokens:
                                                ...
                                                mean_score = per_token_score[mask].sum() / valid_token_count
                        ...
                        # 转换为 tensor
                        if step_accuracies is not None and not isinstance(step_accuracies, torch.Tensor) and len(step_accuracies) > 0:
                            step_accuracies = torch.stack(step_accuracies)  # [S] = [K+1]
                        elif step_accuracies is not None and isinstance(step_accuracies, torch.Tensor):
                            pass
                        else:
                            step_accuracies = None

                        if isinstance(step_penalty_ratios, torch.Tensor):
                            pass
                        elif len(step_penalty_ratios) > 0:
            ...
            if labels is not None:
                ...
                if self.training:
                    ...
                    # -------- [ADD] 跨 micro-batch 累积 base_loss，与 trainer 的 total loss 保持同一尺度 --------
                    try:
                        _accum = getattr(self, "_accumulated_base_loss", None)
                        if _accum is None:
                            self._accumulated_base_loss = base_loss.detach().clone()
                            self._accumulated_base_count = 1
                        else:
                            self._accumulated_base_loss = _accum + base_loss.detach()
                            self._accumulated_base_count = getattr(self, "_accumulated_base_count", 1) + 1
                    except Exception:
                        pass
                    # -------- [ADD END] --------
                    ...
                    if is_rank0 and (wandb.run is not None):
                        try:
                        # train/base_loss 改为由 trainer 从 _accumulated_base_loss 统一记录，保证与 total loss 同一尺度
                            log_dict = {
                                "train/min_weight_penalty_loss": float(min_weight_penalty_loss),
                                "train/lambda_min_weight_penalty": float(current_lambda_min_weight_penalty),
                            }
