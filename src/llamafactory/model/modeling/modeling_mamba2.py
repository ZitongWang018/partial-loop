    ...
    def forward(
        ...
        **kwargs,  # for now we need this for generation
        ...
        if getattr(self.config, "recurrent_model", False):
            # 获取初始 embedding
            if inputs_embeds is None:
                inputs_embeds0 = self.backbone.embeddings(input_ids)
            else:
            ...
            for iteration in range(self.config.more_iterations):
                ...
                if self.config.output_hidden_states and getattr(self.config, "hidden_layer_num", None):
                    if getattr(self.config, "high_memory_mode", False):
                        total = len(mamba2_outputs.hidden_states)
                        selected_layers = [mamba2_outputs.hidden_states[i] for i in range(total - 1, 0, -self.config.hidden_layer_num)]
                        selected_hidden = torch.stack(selected_layers, dim=0)
                        logits = self.lm_head(selected_hidden.to(self.lm_head.weight.dtype))  # shape: [n, batch, seq, vocab_size]
                        probs = torch.softmax(logits / self.config.softmax_temperature, dim=-1).sum(dim=0)
                    else:
                        total = len(mamba2_outputs.hidden_states)
                        selected_layers = [mamba2_outputs.hidden_states[i] for i in range(total - 1, 0, -self.config.hidden_layer_num)]
                        probs = sum(
                            torch.softmax(self.lm_head(layer.to(self.lm_head.weight.dtype)) / self.config.softmax_temperature, dim=-1)
                            for layer in selected_layers
                        )
                else:
                    hidden_states = mamba2_outputs[0] if not return_dict else mamba2_outputs.last_hidden_state
                    logits = self.lm_head(hidden_states.to(self.lm_head.weight.dtype))
                    probs = torch.softmax(logits / self.config.softmax_temperature, dim=-1)
                
                # 确保数据类型一致
                embed_weight = self.backbone.embeddings.weight.to(probs.dtype)
                
                if getattr(self.config, "scale_embeds", False):
                    interpolated_embeds = torch.matmul(probs, embed_weight * embed_scale)
                else:
                    interpolated_embeds = torch.matmul(probs, embed_weight)
                
                if getattr(self.config, "residual_interpolated_embeds", False):
                    current_embeds = current_embeds + interpolated_embeds
                else:
                    current_embeds = inputs_embeds0 + interpolated_embeds
            
            # 使用最终处理后的embedding运行模型一次获取最终输出
            ...
            if not return_dict:
                ...
                hidden_states = final_outputs.last_hidden_state
                
            logits = self.lm_head(hidden_states.to(self.lm_head.weight.dtype)).float()
            
            loss = None
            if labels is not None:
                # 移动labels到正确的设备以支持模型并行
            ...
            return Mamba2CausalLMOutput(
                loss=loss,
                logits=logits,
                cache_params=final_outputs.cache_params,
