...
class Qwen2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, nn.LayerNorm) or isinstance(module, Qwen2RMSNorm):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        if getattr(self.config, "recurrent_model", False):
            ...
            for iteration in range(self.config.more_iterations):
                ...
                if self.config.output_hidden_states and getattr(self.config, "hidden_layer_num", None):
                    if getattr(self.config, "high_memory_mode", False):
                        ...
                        # 假设 outputs.hidden_states[0] 为嵌入，其余每层输出依次排列
                        selected_layers = [outputs.hidden_states[i] for i in range(total - 1, 0, -self.config.hidden_layer_num)]
                        selected_hidden = torch.stack(selected_layers, dim=0)
                        logits = self.lm_head(selected_hidden)  # shape: [n, batch, seq, vocab_size]
                        probs = torch.softmax(logits / self.config.softmax_temperature, dim=-1).sum(dim=0)
                    else:
                        total = len(outputs.hidden_states)
                        selected_layers = [outputs.hidden_states[i] for i in range(total - 1, 0, -self.config.hidden_layer_num)]
                        probs = sum(
                            torch.softmax(self.lm_head(layer) / self.config.softmax_temperature, dim=-1)
                ...
                if getattr(self.config, "scale_embeds", False):
                    interpolated_embeds = torch.matmul(probs, self.model.embed_tokens.weight * embed_scale)
                else:
                    interpolated_embeds = torch.matmul(probs, self.model.embed_tokens.weight)
            ...
            hidden_states = final_outputs[0] if not return_dict else final_outputs.last_hidden_state
