...
class PonderTempCallback(TrainerCallback):
    """
    每步/每个 eval 前更新模型的 ponder 温度，避免在 forward 传 global_step。
    """
    def __init__(self, log_to_wandb: bool = True):
        self.log_to_wandb = log_to_wandb

    def _set_temp(self, model, state):
        # 在开始/评估/结束时多次调用都安全
        total = state.max_steps or state.num_train_epochs or 0
        if hasattr(model, "set_ponder_temperature"):
            model.set_ponder_temperature(global_step=state.global_step, total_steps=total)

    def on_step_begin(self, args, state, control, **kwargs):
        model = kwargs.get("model", None)
        if model is None or not getattr(model, "training", False):
            return
        self._set_temp(model, state)
        # 注释掉ponder/temp的wandb记录，因为一直显示0
        # if self.log_to_wandb and getattr(model, "_wandb_inited", False):
        #     try:
        #         import wandb
        #         wandb.log({"ponder/temp": float(getattr(model, "ponder_temp", 0.0))}, commit=False)
        #     except Exception:
        #         pass

    def on_evaluate(self, args, state, control, **kwargs):
        # 确保 eval 前也刷新一次（有些跑法会在 eval 模式下用最终温度）
        model = kwargs.get("model", None)
        if model is None:
            return
        self._set_temp(model, state)
...
class ReporterCallback(TrainerCallback):
    ...
    def __init__(
        ...
        os.environ["WANDB_PROJECT"] = os.getenv("WANDB_PROJECT", "Ponder2-adaptive")
